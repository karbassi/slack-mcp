...
- `tooling_tokens_rotate` now sends only `refresh_token` (was also sending `client_id`/`client_secret`/`grant_type`, which the method does not accept). `entity_present_details` now sends `trigger_id` plus optional `metadata`/`user_auth_required`/`user_auth_url`/`error` (was sending the unsupported `app_id`/`entity_id`/`entity_type`). `files_upload_v2` now runs Slack's real upload flow via the slack_sdk helper (`files.upload.v2` is not an HTTP method, so the call always failed).
