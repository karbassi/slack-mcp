...
Both are stored in 1Password: `op://vault/item/xoxc-token` and `op://vault/item/xoxd-token`. These tokens last ~1 year (tied to Slack's `d` cookie expiry).
...
- 1Password item ID for tokens: `your-1password-item-id`
